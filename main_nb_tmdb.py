...
# def update_movies():
#     # API endpoint to retrieve movie data
...
#     # Create a table to store the movie data if it doesn't already exist
#     cursor.execute('''
...
#     # Iterate over each movie in the data and insert into the database
#     for movie in data:
#         cursor.execute("""
#             INSERT INTO movies (id, title, release_date, genre)
#             VALUES (?, ?, ?, ?)
#         """, (movie['id'], movie['title'], movie['release_date'], movie['genre']))
...
# schedule.every().day.at("00:00").do(update_movies)
...
# load the movie recommender model and tfidf vectorizer from disk
...
def recommend():
    ...
    imdb_get_title = urllib.request.urlopen('https://www.imdb.com/title/{}/reviews?ref_=tt_ov_rt'.format(imdb_id)).read()
    soup = bs.BeautifulSoup(imdb_get_title,'lxml')
